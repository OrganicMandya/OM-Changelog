...
def main():
    ...
    data = {
        "_comment": ("AUTO-GENERATED by aggregate.py from org changelog.d fragments "
                     "+ releases, rewritten to customer voice by Claude (OM-Infra#86). "
                     "Do not hand-edit — edits are overwritten on the next deploy. "
                     "To change wording, fix the source changelog fragment."),
        "entries": entries,
    }
    with open(content_path, "w", encoding="utf-8") as fh:
        json.dump(data, fh, ensure_ascii=False, indent=2)
        fh.write("\n")
    print(f"aggregate: wrote {len(entries)} entries to content.json.")
    return 0
